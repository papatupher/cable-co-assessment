...
vi.mock("./db", () => {
  const createChain = () => {
    ...
    chain.limit = vi.fn().mockImplementation(() => Promise.resolve(mocks.serviceRequestNotes));

    // Handle the case where limit is not called (the query ends at orderBy)
    const thenMock = vi.fn().mockImplementation((callback) => {
      return callback(mocks.serviceRequestNotes);
    });
    chain.then = thenMock;
...
describe("serviceNotes.add — input validation", () => {
  ...
  it("rejects invalid serviceRequestId type", async () => {
    const caller = appRouter.createCaller(createAdminContext());
    await expect(
      caller.serviceNotes.add({ serviceRequestId: "invalid" as any, content: "Test" })
    ).rejects.toThrow();
  });
...
describe("serviceNotes.list — input validation", () => {
  it("rejects invalid serviceRequestId type", async () => {
    const caller = appRouter.createCaller(createAdminContext());
    await expect(
      caller.serviceNotes.list({ serviceRequestId: "invalid" as any })
    ).rejects.toThrow();
  });
});
...
describe("serviceNotes.delete — input validation", () => {
  it("rejects invalid noteId type", async () => {
    const caller = appRouter.createCaller(createAdminContext());
    await expect(
      caller.serviceNotes.delete({ noteId: "invalid" as any })
    ).rejects.toThrow();
  });
});
